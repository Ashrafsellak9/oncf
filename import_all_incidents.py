#!/usr/bin/env python3
"""
Import de tous les incidents avec colonnes étendues
"""
...
def import_all_incidents():
    """Importer tous les incidents"""
    try:
        # Connexion
        ...
        # Vider la table
        cursor = conn.cursor()
        cursor.execute("TRUNCATE TABLE gpr.ge_evenement RESTART IDENTITY CASCADE;")
        conn.commit()
        print("🗑️ Table vidée")
        ...
        # Types d'incidents
        cursor.execute("SELECT type_id, COUNT(*) FROM gpr.ge_evenement GROUP BY type_id ORDER BY COUNT(*) DESC LIMIT 5;")
